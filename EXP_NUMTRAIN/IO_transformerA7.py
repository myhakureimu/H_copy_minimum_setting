import numpy as np
import os
gpuIdxStr = '0'

for random_seed in [2023,2025]:

    HEAD = 'NUMTRAIN'

    training_content = 'h+xy+z'

    exp_name = 'IOHypothesis'

    icl_k, num_x, num_y = 5, 5, 2

    num_training_hypotheses, num_training_tables = 0, 12358

    max_table_length = 8

    prefix = f'python icl_exp.py --gpu {gpuIdxStr} --random_seed {random_seed} --wandb 1 \
            --HEAD {HEAD} --training_content {training_content} --exp_name {exp_name} \
            --icl_k {icl_k} --num_x {num_x} --num_y {num_y} \
            --num_training_hypotheses {num_training_hypotheses} \
            --max_table_length {max_table_length} --num_training_tables {num_training_tables}'

    epoch_list = [256, 1024, 1024]
    depth_list = [2, 2, 2]#, 8]
    lr_list = [0.0005, 0.001, 0.001]#, 0.00002]
    modelName_list = ['mamba', 'lstm', 'gru']#, 'dual'] #, 'nano']

    wd_list = [0.0005]

    batch_size_list = [16] #, 32, 64]

    loss_on_list = ['all'] #['all', 'y\&z']

    icl_sampling_list = ['iid']

    h_prefix_format_list = [0]

    icl_y_noise_list = [0.0]

    for epoch, depth, lr, modelName in zip(epoch_list, depth_list, lr_list, modelName_list):
        for wd in wd_list:
            for batch_size in batch_size_list:
                for loss_on in loss_on_list:
                    for icl_sampling in icl_sampling_list:
                        for h_prefix_format in h_prefix_format_list:
                            for icl_y_noise in icl_y_noise_list:
                                hyper = f'--depth {depth} --lr {lr} --wd {wd} --batch_size {batch_size} --modelName {modelName} --epochs {epochs} \
                                        --loss_on {loss_on} --icl_sampling {icl_sampling} --h_prefix_format {h_prefix_format} --icl_y_noise {icl_y_noise}'
                                cl = f'{prefix} {hyper}'
                                #print(prefix)
                                #print(hyper)
                                #print(cl)
                                os.system(cl)