...
class NanoGPT(nn.Module):
    ...
    def forward(self, hxy, inds=None):
        zs = label2onehot(hxy, self.n_dims).float()

        input_embeds = self._read_in(zs)
        
        #pos = torch.arange(0, self.config.block_size, dtype=torch.long, device=zs.device).unsqueeze(0) # shape (1, t)
        #pos_emb = self.transformer.wpe(pos) # position embeddings of shape (1, t, n_embd)
